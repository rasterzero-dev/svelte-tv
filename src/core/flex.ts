...
export default function (node: ElementNode): boolean {
  ...
  if (
    ...
    let calculatedSize = totalItemSize + totalGapSize + nodePadding * 2;
  ...
  const doCrossAlign = (
    c: ElementNode,
    idx: number,
    crossCurrentPos: number = nodePadding,
  ) => {
    ...
    const innerCrossSize = Math.max(0, containerCrossSize - nodePadding * 2);
  ...
  if (justify === 'flexStart') {
    if (node.flexWrap === 'wrap' || isWrapReverse) {
      ...
      let crossCurrentPos = isWrapReverse
        ? containerCrossSize - nodePadding - childCrossSizeVar
        : nodePadding;
      ...
      const finalCrossSize = isWrapReverse
        ? containerCrossSize - crossCurrentPos + nodePadding
        : crossCurrentPos + childCrossSizeVar + nodePadding;
    ...
    currentPos =
      nodePadding +
      (containerSize - nodePadding * 2 - (totalItemSize + totalGapSize)) / 2;
    for (let idx = 0; idx < numProcessedChildren; idx++) {
      const c = children[processableChildrenIndices[idx]!] as ElementNode;
      c[prop] = currentPos + childMarginStarts[idx]!;
      currentPos += childTotalMainSizes[idx]! + gap;
