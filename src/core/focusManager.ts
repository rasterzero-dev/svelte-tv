 ...
 */
export const printFocusHistory = (count: number) => {
...
export const focusPathStore = writable<ElementNode[]>([]);

export const focusPath = Object.assign(
  () => get(focusPathStore),
  focusPathStore,
);

export const setFocusPath = (path: ElementNode[]) => focusPathStore.set(path);
...
const DEFAULT_KEY_HOLD_REPEAT_INTERVAL = 90; // ms
type KeyHoldState = {
  timeout?: number;
  interval?: number;
  active: boolean;
};
const keyHoldStates: { [key: KeyNameOrKeyCode]: KeyHoldState | undefined } = {};
  ...
  if (keydown) {
      ...
      if (!state) {
        keyHoldStates[key] = {
          ...
          timeout: window.setTimeout(() => {
            const current = keyHoldStates[key];
            if (!current) return;
            current.active = true;
    ...
    if (state) {
      ...
      if (!state.active) {
        ...
        const mappedKeyHoldEvent =
          keyHoldMapEntries[keyup.key] || keyHoldMapEntries[keyup.keyCode];
        if (mappedKeyHoldEvent) {
          propagateKeyPress(keyup, mappedKeyHoldEvent, false, true);
          return;
        }
      }
    } else {
...
const cleanupKeyHoldStates = () => {
  for (const [key, state] of Object.entries(keyHoldStates)) {
    if (!state) continue;
    if (state.timeout) clearTimeout(state.timeout);
    if (state.interval) clearInterval(state.interval);
    delete keyHoldStates[key];
...
export const createFocusManager = (
  ...
  Config.setActiveElement = (elm) => setActiveElementSignal(elm);

  const delay = keyHoldOptions?.holdThreshold || DEFAULT_KEY_HOLD_THRESHOLD;
  const repeatInterval =
    keyHoldOptions?.repeatInterval || DEFAULT_KEY_HOLD_REPEAT_INTERVAL;
  const runKeyEvent = handleKeyEvents.bind(null, delay, repeatInterval);
  ...
  return () => {
    ...
    cleanupKeyHoldStates();
  };
};

export const useFocusManager = createFocusManager;
